...
### Approach: Static JSON File with System Prompt Injection

**Decision**: Store FAQs in a simple JSON file and inject into Claude's system prompt rather than using vector database or RAG.

**Rationale**:
- **Simplicity**: No additional infrastructure or dependencies required
...
- **Sufficient for Demo**: 20 FAQs fit comfortably in Claude's context window

**Current Scale**: 20 FAQs (~3-4KB of text)

**When to Migrate to Vector Database**:
- FAQ database exceeds 100+ questions (context window constraints)
- Need semantic search across large knowledge base
- Require dynamic FAQ updates without redeployment
- Want to implement true RAG (Retrieval Augmented Generation)

**Alternatives Considered**:
- **Vector Database (Pinecone, Weaviate, Chroma)**: Better for large-scale but overkill for demo; adds complexity
- **Traditional Database (PostgreSQL)**: Better for CRUD but doesn't enable semantic search
- **Hybrid (In-memory Vector Search)**: Considered but unnecessary for current scale

**Migration Path**:
```
JSON File (current) → PostgreSQL (100+ FAQs) → Vector DB (1000+ FAQs, semantic search)
```
...
| Knowledge Base | JSON | Simple, version-controlled, sufficient for demo |
...
- FAQ count exceeds 100 questions
- Need semantic search or similarity matching
