// Shared utilities for chat API routes
// Extracts common functionality used by both streaming and non-streaming endpoints
...
export type GapType = 'no_match' | 'partial_match' | 'off_topic' | 'out_of_scope';
...
//   - out_of_scope: Related to learning but not our platform (NOT ACTIONABLE)
...
export function detectKnowledgeGap(response: string): { isGap: boolean; gapType: GapType } {
...
export async function logFaqGap(
  userMessage: string,
...
export type UserIntent = 'help_seeking' | 'confused' | 'frustrated' | 'off_topic' | 'greeting' | 'feedback' | 'standard_query';
...
export function detectUserIntent(message: string): { intent: UserIntent; confidence: 'high' | 'medium' | 'low' } {
...
// Build intent context hint for system prompt
export function buildIntentContext(userIntent: { intent: UserIntent; confidence: 'high' | 'medium' | 'low' }): string {
  ...
  if (userIntent.intent === 'confused' && userIntent.confidence !== 'low') {
  ...
  if (userIntent.intent === 'off_topic') {
...
export function detectRelevantCategories(userMessage: string): string[] {
  ...
  return relevantCategories.length > 0 ? relevantCategories : Object.keys(CATEGORY_KEYWORDS);
}
...
export interface KnowledgeBaseResult {
  ...
  relevantFaqs: typeof faqData.faqs;
...
export function buildKnowledgeBase(userMessage: string): KnowledgeBaseResult {
  ...
  return {
    knowledgeBase: faqKnowledgeBase + courseCatalogKnowledgeBase,
    relevantFaqs,
    relevantCategories,
  };
}
...
export const SYSTEM_INSTRUCTIONS = `You are a helpful customer service assistant for an AI education company similar to DeepLearning.AI. Your role is to assist users with questions about courses, enrollment, pricing, certificates, and technical support.
